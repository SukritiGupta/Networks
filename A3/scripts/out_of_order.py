...
for i in range(0,len(tcp_packets)):
	if ((tcp_packets.iloc[i]["Destination"] in servers) and (not( tcp_packets.iloc[i]["Destination"] in seen))):
		...
		for i in range(0,len(fromserverToC)):
			temp=fromserverToC[fromserverToC["Time"]>=fromserverToC["Time"].iloc[i]]
			for x in range(0,len(temp)):
				if (temp["seqnum"].iloc[x]<fromserverToC["seqnum"].iloc[i]):
					print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
					print("Seqnum is " + str(temp["seqnum"].iloc[x]) + "got received after ")
					print(" seqnum " + str(fromserverToC["seqnum"].iloc[i]))
					flag=1
					break
			if flag==1:
				break
